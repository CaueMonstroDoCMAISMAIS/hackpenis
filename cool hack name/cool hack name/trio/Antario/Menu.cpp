...
{
    auto mainWindow = std::make_shared<Window>("delta.cc *DEV*", SSize(550, 420), g_Fonts.pFontTahoma8, g_Fonts.pFontTahoma10);
    {
        auto tab1 = std::make_shared<Tab>("A", 2, mainWindow);
        {
            auto sectMain = tab1->AddSection("rage-bot", 1.2435f);
            {
				sectMain->AddText("soon");
            }

            auto fakelag = tab1->AddSection("fake-lag", .4f);
            {
				fakelag->AddCheckBox("slow walk (shift)", &g_Settings.rage.slowwalk);
				fakelag->AddText("slow walk type");
				fakelag->AddCombo("slow walk type", &g_Settings.rage.slowwalktype, std::vector<std::string>{"hitchance", "weapon type"});
				fakelag->AddText("fakelag type");
				fakelag->AddMulti("fakelag", g_Settings.rage.flag_when, std::vector<std::string>{
					"standing",
					"moving",
					"in air"
				});
				fakelag->AddText("fake-lag type");
				fakelag->AddCombo("fake-lag type", &g_Settings.rage.flag_type, std::vector<std::string>{"off", "factor", "dynamic"});
				fakelag->AddText("fakelag packets");
				fakelag->AddSlider("fake-lag packets", "", &g_Settings.rage.flag_packets, 0, 14);
            }
		...
		auto tab2 = std::make_shared<Tab>("H", 2, mainWindow);
		{
			auto legit = tab2->AddSection("legit-bot", 1.2435f);
			{
				legit->AddCheckBox("enable", &g_Settings.rage.lbot_enable);
				legit->AddText("aim bone");
				legit->AddCombo("aim bone", &g_Settings.rage.lbot_hitbox, std::vector<std::string>{"head", "body"});
				legit->AddCheckBox("silent aim", &g_Settings.rage.lbot_silentaim);
				legit->AddText("field of view");
				legit->AddSlider("fov", "", &g_Settings.rage.lbot_fov, 0, 30);
				legit->AddCheckBox("smoothing", &g_Settings.rage.lbot_smoothbox);
				legit->AddText("smooth amount");
				legit->AddSlider("smooth amount", "", &g_Settings.rage.lbot_smooth, 1, 100);
				legit->AddCheckBox("backtrack", &g_Settings.rage.bBacktrack);
			}
		}	mainWindow->AddChild(tab2);

        auto tab3 = std::make_shared<Tab>("D", 2, mainWindow);
        {
			auto players = tab3->AddSection("players", .749f);
			{
			...
			auto playermodels = tab3->AddSection("player models", .497f);
			{
			...
			auto effects = tab3->AddSection("removals", .34f);
			{
			...
			auto world = tab3->AddSection("world", .28);
			{
			...
			auto other = tab3->AddSection("other", .623f);
			{
		...
		auto tab4 = std::make_shared<Tab>("G", 2, mainWindow);
		{
			auto misc1 = tab4->AddSection("misc", .4f);
			{
			...
			auto gameSettings = tab4->AddSection("game settings", .844f);
			{
			...
			auto config = tab4->AddSection("configs", 1.2435f);
			{
