...
void Aimbot::Autostop()
{
	...
	Vector Direction;
	Vector RealView;
	g_Math.VectorAngles_2(Velocity, Direction);
...
{
	if (chance == 0 || g_Settings.rage.Hitchance == 0)
		return true;
	...
	float Seeds = (g_Settings.rage.Hitchance == 1) ? 356.f : 256.f;
	...
	for (int i = 0; i < Seeds; i++)
	{
		...
		g_Math.VectorAngles_3(direction, up, viewanglesSpread);
...
bool ShouldBaim(C_BaseEntity* pEnt) // probably dosnt make sense
{
	...
	if (g_Settings.rage.BaimPitch && BaimShot[pEnt->EntIndex()] && !(pEnt->GetFlags() & FL_ONGROUND))
		return true;

	if (g_Settings.rage.BaimInAir && !(pEnt->GetFlags() & FL_ONGROUND))
		return true;
...
Vector Aimbot::Hitscan(C_BaseEntity* pEnt) // supremeemmemememememe
{
	...
	if (!g_Settings.rage.DelayShot && Velocity > 29.f)
		Velocity = 30.f;
	...
	if (Velocity <= 215.f || Baim)
	{
		...
		if (g_Settings.rage.MultiPoint)
		{
			...
			HeadHeight = g_Settings.rage.HeadScale;
		}

		if (!g_Settings.rage.IgnoreLimbs)
			Velocity = 0.f;
	...
	for (int hitbox : Scan)
	{
		...
		else if (hitbox > 18 && hitbox < 25)
		{
			...
			Radius *= (g_Settings.rage.BodyScale / 100.f);
		...
		if (g_Settings.rage.BaimLethal && hitbox != 0 && hitbox != 25 && hitbox != 26 && hitbox != 27 && Damage >= (pEnt->GetHealth() + 10))
		{
	...
	if (g_Settings.rage.ShotBacktrack && g_LagComp.ShotTick[pEnt->EntIndex()] != -1 && g_Autowall.CanHitFloatingPoint(pEnt->GetHitboxPosition(HITBOX_HEAD, g_LagComp.PlayerRecord[pEnt->EntIndex()].at(g_LagComp.ShotTick[pEnt->EntIndex()]).Matrix) + Vector(0, 0, 1), g::pLocalEntity->GetEyePosition()) && !Baim)
	{
	...
	else if (tempDmg >= g_Settings.rage.Mindmg)
	{
		bestEntDmg = tempDmg;
	...
	else if (g_Settings.rage.PosBacktrack && pPlayerEntityRecord.Velocity >= 29.f && g_Autowall.CanHitFloatingPoint(pEnt->GetHitboxPosition(HITBOX_HEAD, pPlayerEntityRecord.Matrix), g::pLocalEntity->GetEyePosition()))
	{
...
void Aimbot::OnCreateMove()
{
	...
	for (int i = 1; i <= g_pEngine->GetMaxClients(); ++i)
	{
		...
		if (g_LagComp.PlayerRecord[i].size() == 0 || !g::pLocalEntity->IsAlive() || !g_Settings.rage.Aimbot || g_Settings.rage.bBacktrack)
			continue;
	...
	if (shot)
	{
		if (g_Settings.rage.FixShotPitch) // ik it dosnt realy fix much just makes ur pitch go down faster
		{
			g::bSendPacket = true;
			antiaim.anti_aim();
		}
	...
	if (Target)
	{
		...
		if (!(g::pCmd->buttons & IN_ATTACK) && canShoot && HitChance(Target, g::pLocalEntity->GetActiveWeapon(), Angle, Aimpoint, g_Settings.rage.HitchanceValue))
		{
