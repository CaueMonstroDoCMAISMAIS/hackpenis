...
void TraceLine(Vector& vecAbsStart, Vector& vecAbsEnd, unsigned int mask, C_BaseEntity *ignore, CGameTrace *ptr)
{
	...
	g_pEngineTrace->TraceRay(Ray_t(vecAbsStart, vecAbsEnd), mask, &filter, ptr);
}
...
void UTIL_ClipTraceToPlayers(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, ITraceFilter* filter, CGameTrace* tr)
{
...
float Autowall::Damage(const Vector &point)
{
	...
	if (SimulateFireBullet(g::pLocalEntity, g::pLocalEntity2->GetActiveWeapon(), data))
...
bool Autowall::CanHitFloatingPoint(const Vector &point, const Vector &source) // ez
{
	...
	C_BaseCombatWeapon *pWeapon = (C_BaseCombatWeapon*)g::pLocalEntity2->GetActiveWeapon();
