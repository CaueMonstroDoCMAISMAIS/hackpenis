...
void Hooks::Init()
{
	...
	g_pCvar->console_color_printf(g_Settings.config.cMenuColor, "delta.cc *DEV*");
	g_pCvar->console_printf("] ");
	g::pMsg("welcome.\n");
...
bool __fastcall Hooks::CreateMove(IClientMode* thisptr, void* edx, float sample_frametime, CUserCmd* pCmd)
{
	...
	if (!g::fakelag)
		g::bSendPacket = g::pCmd->command_number % 2;
                                 ...
                                 HWND hDestWindowOverride,  const RGNDATA* pDirtyRegion)
{
    ...
    [pDevice]()
    {
        ...
        else
        {
			...
			std::string watermark_text = g_pEngine->IsInGame() && nci ? "delta.cc *DEV* | rtt: " +
				std::to_string(int(nci->GetAvgLatency(FLOW_INCOMING) + nci->GetAvgLatency(FLOW_OUTGOING) * 1000.f)) + "ms" : "delta.cc *DEV*";
